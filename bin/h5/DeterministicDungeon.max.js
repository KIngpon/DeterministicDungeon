...
(function(window,document,Laya){
	...
	*/
	...
	var GameConstant=(function(){
		...
		GameConstant.STAGE_DATA_KEY="dungeon_stage";
		GameConstant.PLAYER_DATA_KEY="dungeon_player";
		return GameConstant;
	})()
	...
	*/
	...
	var MsgConstant=(function(){
		...
		MsgConstant.SHOW_THROUGH_FLOOR="SHOW_THROUGH_FLOOR";
		MsgConstant.REMOVE_THROUGH_FLOOR="REMOVE_THROUGH_FLOOR";
		return MsgConstant;
	})()
	...
	*/
	...
	var GameUtils=(function(){
		...
		GameUtils.getEnemyIconById=function(id){
			...
			if (!ePo)return "";
			var icon=pProxy.getIconById(ePo.icon);
	...
	*@author ...Kanon
	*/
	//class view.components.Damage
	var Damage=(function(){
		var DamageNum;
		function Damage(){};
		__class(Damage,'view.components.Damage');
		Damage.show=function(num,x,y,scale,flag){
			(scale===void 0)&& (scale=1);
			(flag===void 0)&& (flag=false);
			var spt=new DamageNum();
		...
		Damage.showDamageByStr=function(str,x,y,scale){
			(scale===void 0)&& (scale=1);
			var spt=new DamageNum();
		...
		Damage.update=function(){
			...
			for (var i=0;i < Damage.damageAry.length;i++){
				...
				if (spt.y >=640){
					spt.removeSelf();
					Damage.damageAry.splice(i,1);
				}
			}
		}

		Damage.damageAry=[];
		Damage.__init$=function(){
			//class DamageNum extends laya.display.Sprite
			DamageNum=(function(_super){
				function DamageNum(){
					this.vx=0;
					this.vy=0;
					this.g=.6;
				...
				var __proto=DamageNum.prototype;
				/**
				*根据文本内容显示文字
				*@param str 内容
				*/
	...
	*/
	...
	var FloatTips=(function(){
		...
		FloatTips.update=function(){
			...
			for (i=0;i < FloatTips.txtAry.length;i++){
				txt=FloatTips.txtAry[i];
			}
		}
	...
	*/
	...
	var Layer=(function(){
		...
		Layer.init=function(root){
			...
			Layer.GAME_DAMAGE=new Sprite();
			Layer.GAME_LOADNG=new Sprite();
			Layer.STAGE.addChild(Layer.GAME_STAGE);
			Layer.STAGE.addChild(Layer.GAME_UI);
			Layer.STAGE.addChild(Layer.GAME_ALERT);
			Layer.STAGE.addChild(Layer.GAME_DAMAGE);
			Layer.STAGE.addChild(Layer.GAME_LOADNG);
			if (GameConstant.DEBUG){
				Layer.debugTxt=new Label();
				Layer.debugTxt.color="#ff0000";
				Layer.debugTxt.mouseEnabled=false;
				Layer.debugTxt.width=Laya.stage.width;
				Layer.debugTxt.height=Laya.stage.height;
				Layer.debugTxt.fontSize=22;
				Layer.STAGE.addChild(Layer.debugTxt);
			}
		}
		...
		Layer.GAME_LOADNG=null
		Layer.GAME_DAMAGE=null
		Layer.debugTxt=null
		return Layer;
	})()
	...
	*@author ...Kanon
	*/
	//class model.proxy.PicProxy extends mvc.Proxy
	var PicProxy=(function(_super){
		function PicProxy(){
			this.picDict=null;
			this.isLoaded=false;
			PicProxy.__super.call(this);
			this.proxyName="PicProxy";
		}

		__class(PicProxy,'model.proxy.PicProxy',_super);
		var __proto=PicProxy.prototype;
		__proto.initData=function(){
			this.picDict=new Dictionary();
			this.isLoaded=false;
			Laya.loader.load("data/pic.xml",Handler.create(this,function(data){
				var xml=Laya.loader.getRes("data/pic.xml");
				var elementList=xml.getElementsByTagName("pic");
				var count=elementList.length;
				for (var i=0;i < count;++i){
					var childNode=elementList[i];
					var pPo=new PicPo();
		...
		*@return 图片数据
		*/
		__proto.getPicPoById=function(id){
		...
		*@return icon
		*/
		__proto.getIconById=function(id){
		...
		*@return img
		*/
		__proto.getImgById=function(id){
			var pPo=this.getPicPoById(id);
			if (!pPo)return null;
			return pPo.img;
		}
		...
		*@return ani
		*/
		__proto.getAniById=function(id){
			var pPo=this.getPicPoById(id);
			if (!pPo)return null;
			return pPo.ani;
		}

		PicProxy.NAME="PicProxy";
		return PicProxy;
	})(Proxy)
	...
	var PlayerProxy=(function(_super){
		...
		*/
		__proto.parsePlayerData=function(){
			if (!LocalStorage.getJSON("dungeon_player"))return;
			var saveData=LocalStorage.getJSON("dungeon_player");
	...
	*@author ...Kanon
	*/
	//class model.proxy.ResProxy extends mvc.Proxy
	var ResProxy=(function(_super){
		...
		__proto.initData=function(){
			this.sProxy=this.retrieveProxy("StageProxy");
			this.resCount=0;
			this.resList=[];
	...
	var StageProxy=(function(_super){
		...
		__proto.initData=function(){
			...
			Laya.loader.load("data/stage.xml",Handler.create(this,function(data){
				...
				this.curPoints=1;
				this.isLoaded=true;
				console.log(JSON.stringify(LocalStorage.getJSON("dungeon_stage")));
				this.parseStageData();
			}));
		}
		...
		*/
		__proto.randomPointPassByIndex=function(index){
			...
			for (var i=0;i < count;i++){
				if (pVo.passAry[i]==1)
					pVo.up=true;
				else if (pVo.passAry[i]==2)
				pVo.down=true;
				else if (pVo.passAry[i]==3)
				pVo.left=true;
				else if (pVo.passAry[i]==4)
				pVo.right=true;
			}
		}
		...
		*随机所有关卡点的通道
		*/
		__proto.randomAllPointPass=function(){
			for (var i=this.curPointIndex;i < 9;i++){
		...
		*随机关卡点的类型
		*/
		__proto.randomPointType=function(){
			var typeAry=[1,2];
			if (this.isFirstPoint())typeAry=[1,2,3];
			if (this.isBossPoint())typeAry=[1,2,5,4];
			console.log("typeAry",typeAry);
			if (this.step > 1)typeAry.splice(0,this.step-1);
			console.log("typeAry",typeAry);
			var sampleCount=typeAry.length;
		...
		*补完可通过的点
		*/
		__proto.fixPassPoint=function(){
			var count=this.pointsAry.length;
			for (var i=0;i < count;i++){
				var pVo=this.pointsAry[i];
				if (pVo.up){
					if (i !=0 && i !=3 && i !=6){
						var upVo=this.pointsAry[i-1];
						upVo.down=true;
					}
				}
				if (pVo.down){
					if (i !=2 && i !=5 && i !=8){
						var downVo=this.pointsAry[i+1];
						downVo.up=true;
					}
				}
				if (pVo.left){
					if (i !=0 && i !=1 && i !=2){
						var leftVo=this.pointsAry[i-3];
						leftVo.right=true;
					}
				}
				if (pVo.right){
					if (i !=6 && i !=7 && i !=8){
						var rightVo=this.pointsAry[i+3];
						rightVo.left=true;
					}
				}
			}
		...
		*/
		__proto.seach=function(pVo,openList){
			...
			var count=passVoList.length;
			for (i=0;i < count;i++){
				var passVo=passVoList[i];
				if (!passVo.isSeached){
		...
		*@return 关卡点数据
		*/
		__proto.getPointVoByDir=function(index,dir){
			switch (dir){
				case 1:
		...
		*重置点数据的搜索状态
		*/
		__proto.resetPointSeach=function(){
			var count=this.pointsAry.length;
			for (var i=0;i < count;i++){
				var pVo=this.pointsAry[i];
				pVo.isSeached=false;
		...
		*@param type 类型
		*/
		__proto.getPointVoByType=function(type){
			if (!this.pointsAry)return null;
			var count=this.pointsAry.length;
			for (var i=0;i < count;i++){
				var pVo=this.pointsAry[i];
		...
		*/
		__proto.parseStageData=function(){
			...
			o=LocalStorage.getJSON("dungeon_stage"+"_"+this.curLevel+"_"+this.curPoints);
			if (!o)return;
			this.pointsAry=o;
		}
		...
		*/
		__proto.parseStageDataByLevelAndPoints=function(level,points){
			...
			var o=LocalStorage.getJSON("dungeon_stage"+"_"+level+"_"+points);
			this.pointsAry=o;
		}
		...
		*/
		__proto.hasSaveData=function(){
			return LocalStorage.getJSON("dungeon_stage");
		}
	...
	*/
	...
	var GameStageMediator=(function(_super){
		function GameStageMediator(){
			...
			this.resProxy=this.retrieveProxy("ResProxy");
		}
		...
		__proto.listNotificationInterests=function(){
			...
			vect.push("REMOVE_THROUGH_FLOOR");
			return vect;
		}
		...
		__proto.handleNotification=function(notification){
			switch (notification.notificationName){
				...
				case "ALERT_CONFIRM":;
					...
					var prevStagePo=this.curStagePo;
					var nextStagePo;
					var prevStagePointsCount=this.stageProxy.getCurStagePointsCount();
					var nextStagePointsCount=0;
					if (aVo.type==1){
						this.stageProxy.curPoints++;
					...
					this.sendNotification("SHOW_THROUGH_FLOOR",[aVo,prevStagePo,nextStagePo,prevStagePointsCount,nextStagePointsCount]);
					break ;
				case "REMOVE_THROUGH_FLOOR":;
					var aVo=notification.body;
					if(this.stageProxy.hasStageDataByLevelAndPoints(this.stageProxy.curLevel,
						this.stageProxy.curPoints)){
						this.stageProxy.isFirstPointVo=true;
		...
		*重置关卡点数据
		*/
		__proto.resetPointsAry=function(){
			if (this.stageProxy)
	...
	*@author ...Kanon
	*/
	//class view.mediator.LoadingMediator extends mvc.Mediator
	var LoadingMediator=(function(_super){
		function LoadingMediator(){
			this.loading=null;
			LoadingMediator.__super.call(this);
			this.mediatorName="LoadingMediator";
		}

		__class(LoadingMediator,'view.mediator.LoadingMediator',_super);
		...
		*@param pro
		*/
		__proto.updateLoading=function(per){
			if (!this.loading){
	...
	*@author ...Kanon
	*/
	//class view.mediator.SelectNextPointMediator extends mvc.Mediator
	var SelectNextPointMediator=(function(_super){
		function SelectNextPointMediator(){
			...
			SelectNextPointMediator.__super.call(this);
			this.mediatorName="SelectNextPointMediator";
			this.stageProxy=this.retrieveProxy("StageProxy");
		}

		__class(SelectNextPointMediator,'view.mediator.SelectNextPointMediator',_super);
		var __proto=SelectNextPointMediator.prototype;
		__proto.listNotificationInterests=function(){
			var vect=[];
			vect.push("SHOW_SELECT_NEXT_POINT_LAYER");
			vect.push("SELECT_NEXT_POINT");
			return vect;
		}
		...
		__proto.handleNotification=function(notification){
			switch (notification.notificationName){
				case "SHOW_SELECT_NEXT_POINT_LAYER":
					this.initData();
					this.createUI();
					break ;
				case "SELECT_NEXT_POINT":
		...
		*/
		__proto.createUI=function(){
			...
			var upArrow=this.panel.mapSpt.getChildByName("upArrow");
			var leftArrow=this.panel.mapSpt.getChildByName("leftArrow");
			var rightArrow=this.panel.mapSpt.getChildByName("rightArrow");
			var downArrow=this.panel.mapSpt.getChildByName("downArrow");
			if (upArrow)upArrow.on("click",this,this.upArrowClickHandler);
			if (leftArrow)leftArrow.on("click",this,this.leftArrowClickHandler);
			if (rightArrow)rightArrow.on("click",this,this.rightArrowClickHandler);
			if (downArrow)downArrow.on("click",this,this.downArrowClickHandler);
		}
		...
		*/
		...
		__proto.downArrowClickHandler=function(){
		...
		__proto.rightArrowClickHandler=function(){
			if (this.isSelected)return;
			this.stageProxy.curPointVo=
		...
		__proto.leftArrowClickHandler=function(){
			if (this.isSelected)return;
			this.stageProxy.curPointVo=
		...
		__proto.upArrowClickHandler=function(){
			if (this.isSelected)return;
			this.stageProxy.curPointVo=
		...
		__proto.flashingCompleteHandler=function(){
			this.sendNotification("SELECT_NEXT_POINT");
		}

		SelectNextPointMediator.NAME="SelectNextPointMediator";
		return SelectNextPointMediator;
	})(Mediator)
	...
	var SelectStageMediator=(function(_super){
		...
		*/
		__proto.initData=function(){
			...
			this.stageProxy.initPointsAry();
			this.curStagePo=this.stageProxy.getCurStagePo();
			console.log("this.curStagePo",this.curStagePo);
		}
		...
		*/
		__proto.initUI=function(){
			if (!this.selectStageLayer){
				...
				this.selectStageLayer.selectedBtn.on("click",this,this.selectedBtnClickHandler);
				this.selectStageLayer.skipBtn.on("click",this,this.skipBtnClickHandler);
		...
		//跳过
		__proto.skipBtnClickHandler=function(){
			console.log("this.stageProxy.step",this.stageProxy.step);
			console.log("this.stageProxy.maxStep",this.stageProxy.maxStep);
		...
		__proto.selectedBtnClickHandler=function(){
			if (this.stageProxy.step==0 && !this.flashingIsStop){
				if (this.stageProxy.curPointIndex==9 &&
		...
		__proto.flashingCompleteHandler=function(indexValue){
			this.flashingIsStop=false;
	...
	*@author ...Kanon
	*/
	//class view.mediator.ThroughFloorMediator extends mvc.Mediator
	var ThroughFloorMediator=(function(_super){
		function ThroughFloorMediator(){
			this.throughFloorUI=null;
			this.aVo=null;
			this.timer=new Timer();
			ThroughFloorMediator.__super.call(this);
			this.mediatorName="ThroughFloorMediator";
		}

		__class(ThroughFloorMediator,'view.mediator.ThroughFloorMediator',_super);
		var __proto=ThroughFloorMediator.prototype;
		__proto.listNotificationInterests=function(){
			var vect=[];
			vect.push("SHOW_THROUGH_FLOOR");
			vect.push("REMOVE_THROUGH_FLOOR");
			return vect;
		}
		...
		__proto.handleNotification=function(notification){
			switch (notification.notificationName){
				case "SHOW_THROUGH_FLOOR":
					if (!this.throughFloorUI){
						this.throughFloorUI=new ThroughFloorLayerUI();
						Layer.GAME_UI.addChild(this.throughFloorUI);
						var arr=notification.body;
						this.aVo=arr[0];
						var prevStagePo=arr[1];
						var nextStagePo=arr[2];
						var prevStagePointsCount=arr[3];
						var nextStagePointsCount=arr[4];
						if (this.aVo.type==2)
						...
						this.timer.once(2000,this,this.timerCompleteHandler);
					}
					break ;
				case "REMOVE_THROUGH_FLOOR":
					if (this.throughFloorUI){
		...
		__proto.timerCompleteHandler=function(){
			this.sendNotification("REMOVE_THROUGH_FLOOR",this.aVo);
		}

		ThroughFloorMediator.NAME="ThroughFloorMediator";
		return ThroughFloorMediator;
	})(Mediator)
	...
	*/
	...
	var HpBar=(function(_super){
		...
		*死亡效果
		*/
		__proto.deadEffectShow=function(){
			if (!this.flashingTimer)this.flashingTimer=new Timer();
			this.flashingTimer.clear(this,this.flashingLoopHandler);
	...
	*@author Kanon
	*/
	//class view.components.MiniMap extends laya.display.Sprite
	var MiniMap=(function(_super){
		...
		__class(MiniMap,'view.components.MiniMap',_super);
		var __proto=MiniMap.prototype;
		__proto.initUI=function(){
	...
	*@author ...Kanon
	*/
	//class view.components.SlotsPanel extends laya.display.Sprite
	var SlotsPanel=(function(_super){
		function SlotsPanel(){
			this._$3_timer=null;
			this.flashingTimer=null;
			this._index=0;
		...
		__class(SlotsPanel,'view.components.SlotsPanel',_super);
		var __proto=SlotsPanel.prototype;
		...
		*@param isMask 是否启用遮罩
		*/
		__proto.initIcon=function(imgAry,offsetX,offsetY,isMask){
			(offsetX===void 0)&& (offsetX=0);
			(offsetY===void 0)&& (offsetY=0);
			...
			if (count > 9)count=9;
			for (var i=0;i < count;++i){
				var icon=this.contentSpt.getChildByName("m"+(i+1));
				var image=new Image(imgAry[i]);
				icon.addChild(image);
		...
		*@param count 数量
		*/
		__proto.initIconBg=function(name,count){
			this.clearIconBg();
			for (var i=0;i < count;++i){
				var img=this.bgSpt.getChildByName("bg"+(i+1));
				img.skin=name;
				var frameImg=this.frameSpt.getChildByName("frame"+(i+1));
		...
		*@param totalCount 总数
		*/
		__proto.initData=function(totalCount){
			this._index=0;
		...
		*@param isMask 是否遮罩
		*/
		__proto.startNumSlotsByAry=function(numAry,delay,offsetX,offsetY,isMask){
			(offsetX===void 0)&& (offsetX=0);
			(offsetY===void 0)&& (offsetY=0);
			(isMask===void 0)&& (isMask=false);
			if (!numAry)return;
		...
		*@param isMask 是否遮罩
		*/
		__proto.startNumSlotsByNum=function(num,delay,offsetX,offsetY,isMask){
			(offsetX===void 0)&& (offsetX=0);
			(offsetY===void 0)&& (offsetY=0);
			(isMask===void 0)&& (isMask=false);
			if (num > 9)num=9;
			...
			Random.shuffle(this.indexAry);
			var count=this.indexAry.length;
			for (i=0;i < count;++i){
				var index=this.indexAry[i];
				this.iconAry.push("comp/num"+index+".png");
			}
		...
		*@param isMask 是否遮罩
		*/
		__proto.startImageSlots=function(imgAry,frameBg,delay,offsetX,offsetY,scale,isMask){
			(offsetX===void 0)&& (offsetX=0);
			(offsetY===void 0)&& (offsetY=0);
			(scale===void 0)&& (scale=1);
			(isMask===void 0)&& (isMask=false);
		...
		*当前索引
		*/
		__getset(0,__proto,'index',function(){return this._index;});
		/**
		*总数
		*/
		__getset(0,__proto,'totalCount',function(){return this._totalCount;},function(value){
		...
		*选中的值
		*/
		__getset(0,__proto,'indexValue',function(){return this._indexValue;});
		return SlotsPanel;
	})(Sprite)
	...
	var GameStageLayer=(function(_super){
		...
		*/
		__proto.updateStageBg=function(stagePo,stageProxy){
			...
			else if (stageProxy.curPointVo.type==2){
		...
		*/
		__proto.setBgPos=function(level,pointType){
			var bgPosAry;
			if (pointType==2)
			...
			var x=bgPosAry[level-1][0];
			var y=bgPosAry[level-1][1];
			this.bg.x=x;
			this.bg.y=y;
			var frontPosYAry=[0,0,0,0,0,0,0,-31];
		...
		*删除所有敌人
		*/
		__proto.removeAllEnemy=function(){
			var count=this.enemyAry.length;
		...
		*@return 敌人
		*/
		__proto.getEnemyByIndex=function(index){
			if (!this.enemyAry)return null;
	...
	*@author ...Kanon
	*/
	//class view.ui.SelectNextPointLayer extends laya.display.Sprite
	var SelectNextPointLayer=(function(_super){
		function SelectNextPointLayer(){
			this.panel=null;
			this.upStage=null;
			this.downStage=null;
			this.rewardBox=null;
			...
			this.selectIndex=0;
			this.flashingCallBackHandler=null;
			SelectNextPointLayer.__super.call(this);
			this.initUI();
		}

		__class(SelectNextPointLayer,'view.ui.SelectNextPointLayer',_super);
		var __proto=SelectNextPointLayer.prototype;
		/**
		*初始化UI
		*/
		__proto.initUI=function(){
			...
			for (var i=1;i <=9;i++){
				...
				var goodImg=stageIcon.getChildByName("goodImg");
				var badImg=stageIcon.getChildByName("badImg");
				leftSpt.visible=false;
				rightSpt.visible=false;
				upSpt.visible=false;
				downSpt.visible=false;
		...
		*/
		__proto.initPointPass=function(pointAry){
			if (!pointAry)return;
			var count=pointAry.length;
			for (var i=0;i < count;++i){
				var pVo=pointAry[i];
		...
		*/
		__proto.initSlotsBg=function(stagePo){
			if (!stagePo)return;
			var bg=new Image();
			var maskBg=new Image("bg/bgMask.png");
		...
		*@param pVo 关卡点数据
		*/
		__proto.updatePointView=function(pVo){
			if (!pVo)return;
			var stageIcon=this.panel.mapSpt.getChildByName("r"+pVo.index);
			if (!stageIcon)return;
			var leftSpt=stageIcon.getChildByName("leftSpt");
			var rightSpt=stageIcon.getChildByName("rightSpt");
			var upSpt=stageIcon.getChildByName("upSpt");
		...
		*更新当前关卡点的显示
		*/
		__proto.updateCurPointView=function(pVo){
			if (!pVo)return;
		...
		*@param handler
		*/
		__proto.flashing=function(index,handler){
			this.selectIndex=index;
			this.resetAllPointSelect();
			if (!this.flashingTimer)this.flashingTimer=new Timer();
			this.flashingTimer.clear(this,this.flashingLoopHandler);
			this.flashingTimer.loop(40,this,this.flashingLoopHandler);
			this.flashingCallBackHandler=handler;
		}
		...
		*重置所有选中效果
		*/
		__proto.resetAllPointSelect=function(){
			for (var i=1;i <=9;i++){
				var stageIcon=this.panel.mapSpt.getChildByName("r"+i);
				var selectImg=stageIcon.getChildByName("selectImg");
				selectImg.visible=false;
		...
		__proto.flashingLoopHandler=function(){
			var stageIcon=this.panel.mapSpt.getChildByName("r"+this.selectIndex);
			if (stageIcon){
				var selectImg=stageIcon.getChildByName("selectImg");
	...
	*/
	...
	var SelectStageLayer=(function(_super){
		...
		*/
		__proto.stop=function(){
			if (this._$3_timer)this.timer.clear(this,this.loopHandler);
			if (this.flashingTimer)this.flashingTimer.clear(this,this.flashingLoopHandler);
		}
		...
		*/
		__proto.nextStep=function(){
			console.log("ui step",this.step);
			if (this.step==0){
				this.resetAllSelectImg();
				console.log("this.curSelectValue",this.curSelectValue);
				if (this.curSelectValue >=9)
					this.step++;
				else
				this.curSelectValue++;
			}
		...
		*@param pointAry 点数组
		*/
		__proto.updateAllPointPassView=function(pointAry){
			if (!pointAry)return;
			var count=pointAry.length;
			for (var i=0;i < count;i++){
				var pVo=pointAry[i];
		...
		*/
		__proto.updateAllPointTypeView=function(pointAry){
			...
			for (var i=0;i < count;i++){
				...
				var stageIcon=this.panel.mapSpt.getChildByName("r"+pVo.index);
				var selectImg=stageIcon.getChildByName("selectImg");
				console.log("pVo.index",pVo.index);
				console.log("pVo.type",pVo.type);
				if (pVo.type==1){
					this.upStage.x=stageIcon.x+selectImg.width / 2;
					this.upStage.y=stageIcon.y+selectImg.height / 2;
					this.upStage.visible=true;
		...
		*/
		__proto.updatePointValidView=function(openList){
			if (!openList)return;
			this.resetAllSelectImg();
			for (var i=1;i <=9;i++){
				var stageIcon=this.panel.mapSpt.getChildByName("r"+i);
	...
	var Animation=(function(_super){
		...
		*/
		...
		__proto._parseGraphicAnimation=function(animationData){
			return GraphicAnimation1.parseAnimationData(animationData);
		}

		/**@private */
		__proto._parseGraphicAnimationByData=function(animationObject){
			return GraphicAnimation1.parseAnimationByData(animationObject);
		}
	...
	*/
	...
	var GraphicAnimation1=(function(_super){
		var GraphicNode;
		...
		__class(GraphicAnimation,'laya.utils.GraphicAnimation',_super,'GraphicAnimation1');
	...
	var LoadingLayerUI=(function(_super){
		function LoadingLayerUI(){
			this.perTxt=null;
			LoadingLayerUI.__super.call(this);
		}

		__class(LoadingLayerUI,'ui.GameStage.LoadingLayerUI',_super);
		var __proto=LoadingLayerUI.prototype;
		__proto.createChildren=function(){
			View.regComponent("Text",Text);
			laya.ui.Component.prototype.createChildren.call(this);
			this.createView(LoadingLayerUI.uiView);
		}

		__static(LoadingLayerUI,
		['uiView',function(){return this.uiView={"type":"View","props":{"width":1136,"height":640},"child":[{"type":"Text","props":{"y":327.2096774193548,"x":562.3548387096774,"width":450,"var":"perTxt","text":"正在加载图片..... 10%","pivotY":38.70967741935482,"pivotX":219.35483870967744,"height":71,"fontSize":"30","font":"Microsoft YaHei","color":"#ffffff","align":"center"}}]};}
		]);
	...
	var SelectStageLayerUI=(function(_super){
		...
		['uiView',function(){return this.uiView={"type":"View","props":{"width":1136,"name":"selectImg","height":640},"child":[{"type":"Rect","props":{"y":0,"x":0,"width":1136,"lineWidth":1,"height":640,"fillColor":"#000000"}},{"type":"Sprite","props":{"y":162,"x":229,"width":640,"var":"bgSpt","height":394}},{"type":"Button","props":{"y":504,"x":960,"var":"selectBtn","stateNum":"3","skin":"btn/selectBtn.png","anchorY":1,"anchorX":0.5}},{"type":"Image","props":{"y":264,"x":917,"var":"handImg","skin":"comp/hand.png"}},{"type":"Sprite","props":{"y":207,"x":291,"width":518,"var":"mapSpt","height":322},"child":[{"type":"Sprite","props":{"y":33,"x":85,"name":"r1"},"child":[{"type":"Rect","props":{"width":110,"lineWidth":1,"height":78,"fillColor":"#000000"}},{"type":"Image","props":{"y":0,"x":0,"width":110,"skin":"frame/stageSlotsSelectedBg.png","name":"selectImg","height":78}},{"type":"Sprite","props":{"y":30,"x":-15,"name":"leftSpt"},"child":[{"type":"Rect","props":{"width":20,"lineWidth":1,"height":20,"fillColor":"#000000"}}]},{"type":"Sprite","props":{"y":-13,"x":45,"name":"upSpt"},"child":[{"type":"Rect","props":{"width":20,"lineWidth":1,"height":20,"fillColor":"#000000"}}]},{"type":"Sprite","props":{"y":71,"x":45,"name":"downSpt"},"child":[{"type":"Rect","props":{"width":20,"lineWidth":1,"height":20,"fillColor":"#000000"}}]},{"type":"Sprite","props":{"y":30,"x":105,"name":"rightSpt"},"child":[{"type":"Rect","props":{"width":20,"lineWidth":1,"height":20,"fillColor":"#000000"}}]},{"type":"Image","props":{"y":42,"x":76,"skin":"btn/okBtn.png","scaleY":0.4,"scaleX":0.4,"name":"goodImg"}},{"type":"Image","props":{"y":44,"x":76,"skin":"btn/closeBtn.png","scaleY":0.4,"scaleX":0.4,"name":"badImg"}}]},{"type":"Sprite","props":{"y":33,"x":205,"name":"r4"},"child":[{"type":"Rect","props":{"width":110,"lineWidth":1,"height":78,"fillColor":"#000000"}},{"type":"Image","props":{"y":0,"x":0,"width":110,"skin":"frame/stageSlotsSelectedBg.png","name":"selectImg","height":78}},{"type":"Sprite","props":{"y":30,"x":-15,"name":"leftSpt"},"child":[{"type":"Rect","props":{"width":20,"lineWidth":1,"height":20,"fillColor":"#000000"}}]},{"type":"Sprite","props":{"y":-13,"x":45,"name":"upSpt"},"child":[{"type":"Rect","props":{"width":20,"lineWidth":1,"height":20,"fillColor":"#000000"}}]},{"type":"Sprite","props":{"y":71,"x":45,"name":"downSpt"},"child":[{"type":"Rect","props":{"width":20,"lineWidth":1,"height":20,"fillColor":"#000000"}}]},{"type":"Sprite","props":{"y":30,"x":105,"name":"rightSpt"},"child":[{"type":"Rect","props":{"width":20,"lineWidth":1,"height":20,"fillColor":"#000000"}}]},{"type":"Image","props":{"y":42,"x":76,"skin":"btn/okBtn.png","scaleY":0.4,"scaleX":0.4,"name":"goodImg"}},{"type":"Image","props":{"y":44,"x":76,"skin":"btn/closeBtn.png","scaleY":0.4,"scaleX":0.4,"name":"badImg"}}]},{"type":"Sprite","props":{"y":33,"x":325,"name":"r7"},"child":[{"type":"Rect","props":{"width":110,"lineWidth":1,"height":78,"fillColor":"#000000"}},{"type":"Image","props":{"y":0,"x":0,"width":110,"skin":"frame/stageSlotsSelectedBg.png","name":"selectImg","height":78}},{"type":"Sprite","props":{"y":30,"x":-15,"name":"leftSpt"},"child":[{"type":"Rect","props":{"width":20,"lineWidth":1,"height":20,"fillColor":"#000000"}}]},{"type":"Sprite","props":{"y":-13,"x":45,"name":"upSpt"},"child":[{"type":"Rect","props":{"width":20,"lineWidth":1,"height":20,"fillColor":"#000000"}}]},{"type":"Sprite","props":{"y":71,"x":45,"name":"downSpt"},"child":[{"type":"Rect","props":{"width":20,"lineWidth":1,"height":20,"fillColor":"#000000"}}]},{"type":"Sprite","props":{"y":30,"x":105,"name":"rightSpt"},"child":[{"type":"Rect","props":{"width":20,"lineWidth":1,"height":20,"fillColor":"#000000"}}]},{"type":"Image","props":{"y":42,"x":76,"skin":"btn/okBtn.png","scaleY":0.4,"scaleX":0.4,"name":"goodImg"}},{"type":"Image","props":{"y":44,"x":76,"skin":"btn/closeBtn.png","scaleY":0.4,"scaleX":0.4,"name":"badImg"}}]},{"type":"Sprite","props":{"y":121,"x":85,"name":"r2"},"child":[{"type":"Rect","props":{"width":110,"lineWidth":1,"height":78,"fillColor":"#000000"}},{"type":"Image","props":{"y":0,"x":0,"width":110,"skin":"frame/stageSlotsSelectedBg.png","name":"selectImg","height":78}},{"type":"Sprite","props":{"y":30,"x":-15,"name":"leftSpt"},"child":[{"type":"Rect","props":{"width":20,"lineWidth":1,"height":20,"fillColor":"#000000"}}]},{"type":"Sprite","props":{"y":-13,"x":45,"name":"upSpt"},"child":[{"type":"Rect","props":{"width":20,"lineWidth":1,"height":20,"fillColor":"#000000"}}]},{"type":"Sprite","props":{"y":71,"x":45,"name":"downSpt"},"child":[{"type":"Rect","props":{"width":20,"lineWidth":1,"height":20,"fillColor":"#000000"}}]},{"type":"Sprite","props":{"y":30,"x":105,"name":"rightSpt"},"child":[{"type":"Rect","props":{"width":20,"lineWidth":1,"height":20,"fillColor":"#000000"}}]},{"type":"Image","props":{"y":42,"x":76,"skin":"btn/okBtn.png","scaleY":0.4,"scaleX":0.4,"name":"goodImg"}},{"type":"Image","props":{"y":44,"x":76,"skin":"btn/closeBtn.png","scaleY":0.4,"scaleX":0.4,"name":"badImg"}}]},{"type":"Sprite","props":{"y":121,"x":205,"name":"r5"},"child":[{"type":"Rect","props":{"width":110,"lineWidth":1,"height":78,"fillColor":"#000000"}},{"type":"Image","props":{"y":0,"x":0,"width":110,"skin":"frame/stageSlotsSelectedBg.png","name":"selectImg","height":78}},{"type":"Sprite","props":{"y":30,"x":-15,"name":"leftSpt"},"child":[{"type":"Rect","props":{"width":20,"lineWidth":1,"height":20,"fillColor":"#000000"}}]},{"type":"Sprite","props":{"y":-13,"x":45,"name":"upSpt"},"child":[{"type":"Rect","props":{"width":20,"lineWidth":1,"height":20,"fillColor":"#000000"}}]},{"type":"Sprite","props":{"y":71,"x":45,"name":"downSpt"},"child":[{"type":"Rect","props":{"width":20,"lineWidth":1,"height":20,"fillColor":"#000000"}}]},{"type":"Sprite","props":{"y":30,"x":105,"name":"rightSpt"},"child":[{"type":"Rect","props":{"width":20,"lineWidth":1,"height":20,"fillColor":"#000000"}}]},{"type":"Image","props":{"y":42,"x":76,"skin":"btn/okBtn.png","scaleY":0.4,"scaleX":0.4,"name":"goodImg"}},{"type":"Image","props":{"y":44,"x":76,"skin":"btn/closeBtn.png","scaleY":0.4,"scaleX":0.4,"name":"badImg"}}]},{"type":"Sprite","props":{"y":121,"x":325,"name":"r8"},"child":[{"type":"Rect","props":{"width":110,"lineWidth":1,"height":78,"fillColor":"#000000"}},{"type":"Image","props":{"y":0,"x":0,"width":110,"skin":"frame/stageSlotsSelectedBg.png","name":"selectImg","height":78}},{"type":"Sprite","props":{"y":30,"x":-15,"name":"leftSpt"},"child":[{"type":"Rect","props":{"width":20,"lineWidth":1,"height":20,"fillColor":"#000000"}}]},{"type":"Sprite","props":{"y":-13,"x":45,"name":"upSpt"},"child":[{"type":"Rect","props":{"width":20,"lineWidth":1,"height":20,"fillColor":"#000000"}}]},{"type":"Sprite","props":{"y":71,"x":45,"name":"downSpt"},"child":[{"type":"Rect","props":{"width":20,"lineWidth":1,"height":20,"fillColor":"#000000"}}]},{"type":"Sprite","props":{"y":30,"x":105,"name":"rightSpt"},"child":[{"type":"Rect","props":{"width":20,"lineWidth":1,"height":20,"fillColor":"#000000"}}]},{"type":"Image","props":{"y":42,"x":76,"skin":"btn/okBtn.png","scaleY":0.4,"scaleX":0.4,"name":"goodImg"}},{"type":"Image","props":{"y":44,"x":76,"skin":"btn/closeBtn.png","scaleY":0.4,"scaleX":0.4,"name":"badImg"}}]},{"type":"Sprite","props":{"y":209,"x":85,"name":"r3"},"child":[{"type":"Rect","props":{"width":110,"lineWidth":1,"height":78,"fillColor":"#000000"}},{"type":"Image","props":{"y":0,"x":0,"width":110,"skin":"frame/stageSlotsSelectedBg.png","name":"selectImg","height":78}},{"type":"Sprite","props":{"y":30,"x":-15,"name":"leftSpt"},"child":[{"type":"Rect","props":{"width":20,"lineWidth":1,"height":20,"fillColor":"#000000"}}]},{"type":"Sprite","props":{"y":-13,"x":45,"name":"upSpt"},"child":[{"type":"Rect","props":{"width":20,"lineWidth":1,"height":20,"fillColor":"#000000"}}]},{"type":"Sprite","props":{"y":71,"x":45,"name":"downSpt"},"child":[{"type":"Rect","props":{"width":20,"lineWidth":1,"height":20,"fillColor":"#000000"}}]},{"type":"Sprite","props":{"y":30,"x":105,"name":"rightSpt"},"child":[{"type":"Rect","props":{"width":20,"lineWidth":1,"height":20,"fillColor":"#000000"}}]},{"type":"Image","props":{"y":42,"x":76,"skin":"btn/okBtn.png","scaleY":0.4,"scaleX":0.4,"name":"goodImg"}},{"type":"Image","props":{"y":44,"x":76,"skin":"btn/closeBtn.png","scaleY":0.4,"scaleX":0.4,"name":"badImg"}}]},{"type":"Sprite","props":{"y":209,"x":205,"name":"r6"},"child":[{"type":"Rect","props":{"width":110,"lineWidth":1,"height":78,"fillColor":"#000000"}},{"type":"Image","props":{"y":0,"x":0,"width":110,"skin":"frame/stageSlotsSelectedBg.png","name":"selectImg","height":78}},{"type":"Sprite","props":{"y":30,"x":-15,"name":"leftSpt"},"child":[{"type":"Rect","props":{"width":20,"lineWidth":1,"height":20,"fillColor":"#000000"}}]},{"type":"Sprite","props":{"y":-13,"x":45,"name":"upSpt"},"child":[{"type":"Rect","props":{"width":20,"lineWidth":1,"height":20,"fillColor":"#000000"}}]},{"type":"Sprite","props":{"y":71,"x":45,"name":"downSpt"},"child":[{"type":"Rect","props":{"width":20,"lineWidth":1,"height":20,"fillColor":"#000000"}}]},{"type":"Sprite","props":{"y":30,"x":105,"name":"rightSpt"},"child":[{"type":"Rect","props":{"width":20,"lineWidth":1,"height":20,"fillColor":"#000000"}}]},{"type":"Image","props":{"y":42,"x":76,"skin":"btn/okBtn.png","scaleY":0.4,"scaleX":0.4,"name":"goodImg"}},{"type":"Image","props":{"y":44,"x":76,"skin":"btn/closeBtn.png","scaleY":0.4,"scaleX":0.4,"name":"badImg"}}]},{"type":"Sprite","props":{"y":209,"x":325,"name":"r9"},"child":[{"type":"Rect","props":{"width":110,"lineWidth":1,"height":78,"fillColor":"#000000"}},{"type":"Image","props":{"y":0,"x":0,"width":110,"skin":"frame/stageSlotsSelectedBg.png","name":"selectImg","height":78}},{"type":"Sprite","props":{"y":30,"x":-15,"name":"leftSpt"},"child":[{"type":"Rect","props":{"width":20,"lineWidth":1,"height":20,"fillColor":"#000000"}}]},{"type":"Sprite","props":{"y":-13,"x":45,"name":"upSpt"},"child":[{"type":"Rect","props":{"width":20,"lineWidth":1,"height":20,"fillColor":"#000000"}}]},{"type":"Sprite","props":{"y":71,"x":45,"name":"downSpt"},"child":[{"type":"Rect","props":{"width":20,"lineWidth":1,"height":20,"fillColor":"#000000"}}]},{"type":"Sprite","props":{"y":30,"x":105,"name":"rightSpt"},"child":[{"type":"Rect","props":{"width":20,"lineWidth":1,"height":20,"fillColor":"#000000"}}]},{"type":"Image","props":{"y":42,"x":76,"skin":"btn/okBtn.png","scaleY":0.4,"scaleX":0.4,"name":"goodImg"}},{"type":"Image","props":{"y":44,"x":76,"skin":"btn/closeBtn.png","scaleY":0.4,"scaleX":0.4,"name":"badImg"}}]},{"type":"Image","props":{"y":107,"x":-62,"var":"downStage","skin":"comp/downStage.png"}},{"type":"Image","props":{"y":27,"x":-70,"var":"upStage","skin":"comp/upStage.png"}},{"type":"Image","props":{"y":177,"x":-74,"var":"rewardBox","skin":"comp/rewardBox.png"}},{"type":"Image","props":{"y":258,"x":-75,"var":"bossImg","skin":"comp/bossIcon.png"}},{"type":"Image","props":{"y":354,"x":-79,"var":"bossRewardBox","skin":"comp/bossBox.png"}}]},{"type":"Image","props":{"y":112,"x":549,"skin":"comp/selectStageTitle.png","anchorX":0.5}},{"type":"Image","props":{"y":181,"x":483,"skin":"frame/stageNumBg.png"}},{"type":"Button","props":{"y":523,"x":906,"var":"skipBtn","stateNum":"1","skin":"btn/jumpBtn.png"}},{"type":"Image","props":{"y":526,"x":920,"var":"skipWord","skin":"btn/jumpWord.png","mouseEnabled":false}},{"type":"Label","props":{"y":180,"x":369,"width":364,"var":"stageNumTxt","valign":"top","strokeColor":"#000000","stroke":0,"height":37,"fontSize":20,"font":"Microsoft YaHei","color":"#ffffff","align":"center"}},{"type":"Label","props":{"y":515,"x":370,"width":364,"var":"selectDesTxt","text":"选择房间通道","strokeColor":"#080808","stroke":3,"height":37,"fontSize":22,"font":"Microsoft YaHei","color":"#ffffff","align":"center"}}]};}
		]);
	...
	*/
	...
	//class ui.GameStage.ThroughFloorLayerUI extends laya.ui.View
	var ThroughFloorLayerUI=(function(_super){
		function ThroughFloorLayerUI(){
			this.floorBg=null;
			this.nameTxt=null;
			ThroughFloorLayerUI.__super.call(this);
		}

		__class(ThroughFloorLayerUI,'ui.GameStage.ThroughFloorLayerUI',_super);
		var __proto=ThroughFloorLayerUI.prototype;
		__proto.createChildren=function(){
			laya.ui.Component.prototype.createChildren.call(this);
			this.createView(ThroughFloorLayerUI.uiView);
		}

		__static(ThroughFloorLayerUI,
		['uiView',function(){return this.uiView={"type":"View","props":{"width":1136,"height":640},"child":[{"type":"Sprite","props":{"y":0,"x":0,"width":1136,"height":640,"alpha":0.6},"child":[{"type":"Rect","props":{"y":0,"x":0,"width":1136,"lineWidth":1,"height":640,"fillColor":"#000000"}}]},{"type":"Image","props":{"y":327,"x":564,"width":314,"var":"floorBg","skin":"comp/floor.png","scaleX":-1,"pivotY":139.6551724137931,"pivotX":153.44827586206895,"height":265}},{"type":"Label","props":{"y":478,"x":421.9655172413793,"width":630,"var":"nameTxt","text":"通过泥土1-1前往泥土1-2","pivotY":20.68965517241378,"pivotX":168.9655172413793,"height":30,"fontSize":30,"font":"Microsoft YaHei","color":"#ffffff","align":"center"}}]};}
		]);
		return ThroughFloorLayerUI;
	})(View)
	...
	*/
	...
	//class ui.GameStage.SlotsPanelUI extends laya.ui.Dialog
	var SlotsPanelUI=(function(_super){
		function SlotsPanelUI(){
			this.titleTxt=null;
			this.selectBtn=null;
			this.bgSpt=null;
			this.contentSpt=null;
			...
			this.selectedImg=null;
			this.handImg=null;
			SlotsPanelUI.__super.call(this);
		}

		__class(SlotsPanelUI,'ui.GameStage.SlotsPanelUI',_super);
		var __proto=SlotsPanelUI.prototype;
		__proto.createChildren=function(){
			laya.ui.Component.prototype.createChildren.call(this);
			this.createView(SlotsPanelUI.uiView);
		}
